...
import { Users, Briefcase, BarChart2, LogOut, LayoutDashboard, Plus, Search, Edit2, Trash2 } from 'lucide-react';
...
const AdminOpportunities = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingOpp) {
        const response = await api.patch(`/admin/opportunities/${editingOpp._id}`, data);
        setOpportunities(opportunities.map(op => op._id === editingOpp._id ? response.data.data.opportunity : op));
    ...
    <div className="flex bg-gray-50">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 hidden md:flex flex-col min-h-full">
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          <Link to="/admin/opportunities" className="flex items-center gap-3 px-4 py-3 bg-primary-50 text-primary-700 rounded-lg font-medium">
      ...
      <main className="flex-1 p-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">Create, edit, and manage opportunities.</p>
          </div>
          <button 
            onClick={() => openModal()}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredOpportunities.map((opp) => (
                  <tr key={opp._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${opp.type === 'full-time' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
                        {opp.type}
                      </span>
                    </td>
