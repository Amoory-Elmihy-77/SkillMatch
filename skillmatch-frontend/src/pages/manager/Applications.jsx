...
import { Users, Filter, X, CheckCircle, XCircle, FileText, Clock } from 'lucide-react';
...
const ManagerApplications = () => {
    ...
    const handleUpdateStatus = async (applicationId, status, notes = '') => {
        setUpdating(true);
        try {
            await api.patch(`/applications/${applicationId}/status`, { status, notes });
    ...
    const getStatusConfig = (status) => {
        switch (status) {
            case 'pending':
                return { label: 'Pending', icon: <Clock className="w-5 h-5" />, className: 'bg-yellow-50 text-yellow-700 border-yellow-200' };
            case 'reviewed':
                return { label: 'Reviewed', icon: <FileText className="w-5 h-5" />, className: 'bg-blue-50 text-blue-700 border-blue-200' };
            case 'accepted':
                return { label: 'Accepted', icon: <CheckCircle className="w-5 h-5" />, className: 'bg-green-50 text-green-700 border-green-200' };
            case 'rejected':
                return { label: 'Rejected', icon: <XCircle className="w-5 h-5" />, className: 'bg-red-50 text-red-700 border-red-200' };
            default:
                return { label: status, icon: <FileText className="w-5 h-5" />, className: 'bg-gray-50 text-gray-700 border-gray-200' };
        }
    };
                                        ...
                                        <div className="flex items-start gap-4 flex-1">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-center gap-3 mb-2">
                                                    ...
                                                    <div className={`flex items-center gap-2 px-3 py-1 rounded-lg border text-sm ${statusConfig.className}`}>
                                                        {statusConfig.icon}
                    ...
                    <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                />
                                <div>
                                    <h3 className="text-xl font-bold text-gray-900">{selectedApp.applicant?.name}</h3>
                                    <p className="text-gray-600">{selectedApp.applicant?.email}</p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <p className="text-lg font-semibold text-gray-900">{selectedApp.opportunity?.title}</p>
                            </div>

                            {/* Application Date */}
                            <div>
                                <p className="text-sm font-medium text-gray-700 mb-1">Application Date:</p>
